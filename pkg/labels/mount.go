...
// SetMount centralizes the logic for marshaling and chunking mount metadata into a label map.
func SetMount(m map[string]string, mounts any) error {
	if mounts == nil {
		return nil
	}
	...
	if string(jsonMountBytes) == "null" || string(jsonMountBytes) == "[]" {
		return nil
	}
...
// GetMount extracts and reassembles indexed mount metadata back into a single global JSON array string
